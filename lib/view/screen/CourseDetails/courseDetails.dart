...
import 'package:learning_managment_system/view/widget/profile/showdialog.dart';
...
class CourseDetailsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ProfileControllerImp controllerImp = Get.put(ProfileControllerImp());
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        leading: IconButton(
          ...
          onPressed: () {
            controllersub.fetchCourses(controllersub.subId);
            ...
            SliverFillRemaining(
              ...
              child: Column(
                ...
                children: [
                  ...
                  CustomCourseDetails(
                    title: 'Created from',
                    text: '${courseDetails.createdFrom ?? ''}',
                  ...
                  CustomCourseDetails(
                    title: 'Total likes',
                    text: '${courseDetails.totalLikes.toString() ?? ''}',
                    iconData: Icons.favorite_outline,
                  ),
                  SizedBox(height: 15),
                  CustomCourseDetails(
                    title: 'Number of videos',
                    text: '${controller.course!.videos!.length.toString()}',
                    iconData: Icons.video_collection_outlined,
                  ),
                  SizedBox(height: 15),
                  CustomCourseDetails(
                    title: 'Number of quizzes',
                    text: '${controller.course!.quizzes!.length.toString()}',
                    iconData: Icons.quiz_outlined,
                  ),
                  SizedBox(height: 15),
                  CustomCourseDetails(
                    title: 'Price',
                    text: '${courseDetails.price.toString() ?? ''}',
                    ...
                    child: Stack(
                      children: [
                        ...
                        TeacherDetails(
                          ...
                          widget: controller.teacher!.image!.isEmpty ||
                                  controller.teacher!.image == null
                              ? Image.asset(
                  ...
                  Padding(
                    ...
                    child: courseDetails.studentHasEnrolled == 0
                        ? CustomButton(
                            textButton: 'Enroll',
                            onTap: () {
                              controller.enroll(courseDetails.id!);
                              controllerImp.fetchProfile();
                            },
                          )
                        : CustomButton(
                            textButton: 'Unenroll',
                            onTap: () {
                              controller.unEnroll(courseDetails.id!);
                              controllerImp.fetchProfile();
                            },
                          ),
                  ),
                  Center(
                    child: MaterialButton(
                      onPressed: () {
                        print(
                            'studentHasEnrolledddddddddddddddddd ${courseDetails.studentHasEnrolled}');
                        if (courseDetails.studentHasEnrolled == 1) {
                          Get.to(QuizVideos());
                        } else {
                          Get.defaultDialog(
                              title:
                                  "You can't see the content if you don't enroll",
                              content: Text(''),
